...
- **Bandarmology** — Bandar accumulation/distribution detection via IDX Market Intelligence API
- **Smart Money Flow** — institutional and foreign investor movement tracking
- **Pump & Dump Detection** — market manipulation risk signals
- **Foreign Ownership** — foreign investor ownership trend monitoring
- *(Powered by IDX Market Intelligence API — 1000 req/month free; safe no-op when key not set)*
...
- **Research Only**: This tool is designed for research and educational purposes only
- **Not Financial Advice**: Analysis generated by AI agents is not financial advice
- **No Live Trading**: This system does not connect to any broker or execute trades
- **Data Limitations**: Uses yfinance and Alpha Vantage which may have limited coverage for IDX (`.JK`) tickers compared to IDX-native data providers
- **IDX Data Note**: Set `IDX_RAPIDAPI_KEY` to enable Bandarmology, Smart Money Flow, Pump & Dump Detection, and Foreign Ownership data from the IDX Market Intelligence API (1000 req/month free tier)
...
| `IDX_RAPIDAPI_KEY` | No | RapidAPI key for IDX-exclusive Bandarmology/Smart Money features (1000 req/month free) |
...
> **Data availability note**: yfinance coverage for `.JK` (IDX), `.T` (TSE), `.HK` (HKEX), and `.TO` (TSX) tickers may be incomplete compared to US markets. Verify data availability for your specific ticker before relying on the analysis.
