...
function logClass(log: string): string {
  for (const [prefix, cls] of Object.entries(LOG_COLOR)) {
    if (log.startsWith(prefix)) return cls
  }
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const tick = () => setWibClock(fmt.format(new Date()))
    tick()
  ...
  // Shared log sheet content (used by both mobile bottom sheet and tablet log sheet)
  const LogSheetContent = (
    <>
      <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
        <span className="text-xs font-mono text-zinc-400 uppercase tracking-widest">Live Log ({logs.length})</span>
        <button onClick={() => setLogSheetOpen(false)} className="text-zinc-400 text-xl leading-none">×</button>
      </div>
      <div className="overflow-y-auto flex-1 px-4 py-2 text-xs font-mono space-y-0.5 pb-12 scrollbar-thin">
        {logs.map((log, i) => (
  ...
  return (
    <div className="dot-grid bg-[#09090b]" style={{ backgroundAttachment: "fixed" }}>
      ...
      <div className="flex h-screen pt-12">
        ...
        <aside className="hidden md:flex flex-col w-56 lg:w-64 shrink-0
          ...
          <div className="p-4 space-y-4">
            ...
            {/* Status text */}
            {status && (
              <p className="text-[11px] font-mono text-zinc-600 break-words px-1">{status}</p>
            )}
          </div>
        </aside>
        ...
        <main className={`flex-1 overflow-y-auto px-4 lg:px-6 py-4 min-w-0
          ${isMobile && logs.length > 0 ? "pb-14" : "pb-8"}`}>
          ...
          <div className="glass-panel rounded-lg p-4">
            <div className="flex flex-wrap lg:flex-nowrap gap-3 items-end">
              ...
              <div className="shrink-0">
                ...
                <p className="text-[11px] text-zinc-700 mt-1 hidden lg:block">
                  US: NVDA &middot; ID: BBCA.JK &middot; JP: 7203.T
                </p>
              </div>
              ...
              <div className="shrink-0">
                ...
                <input
                  ...
                  className="bg-zinc-900 border border-zinc-700 rounded px-3 py-2 text-sm
                    font-mono text-zinc-100 focus:outline-none focus:border-zinc-500"
                />
              </div>

              {/* Debate Rounds */}
              <div className="shrink-0">
                <label className="text-xs text-zinc-500 uppercase tracking-wider block mb-1">Rounds</label>
              ...
              <div className="flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  className="bg-zinc-900 border border-zinc-700 rounded px-3 py-1.5 text-xs
                    font-mono text-zinc-100 w-full focus:outline-none focus:border-zinc-500 mb-1"
                />
                <select
                  value={model}
                  onChange={e => {
                  ...
                  className="bg-zinc-900 border border-zinc-700 rounded px-3 py-2 text-xs
                    font-mono text-zinc-100 w-full focus:outline-none focus:border-zinc-500"
                >
              ...
              <div className="flex gap-2 shrink-0">
                <button
                  onClick={handleRun}
                  disabled={running}
                  className="px-6 py-2 rounded font-bold text-sm tracking-wider transition-all duration-200"
                  style={{
                  ...
                  onMouseEnter={e => {
                    if (!running) (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 20px rgba(34,197,94,0.3)"
                  }}
                  onMouseLeave={e => {
                ...
                >
                  {running
                    ? <span className="flex items-center gap-2"><span className="inline-block animate-spin">⟳</span>RUNNING...</span>
                    : "▶ RUN ANALYSIS"
                  }
                </button>
                {running && (
            ...
            {/* Estimator */}
            {!running && (
              <p className="mt-2 text-[11px] font-mono text-zinc-600 italic">
                EST: ~{estMins} min{estCost != null && estCost > 0 ? ` · ~$${estCost.toFixed(4)}` : ""} ({estSource})
              </p>
            )}
          </div>
          ...
          {jobId && (
            <div className="mt-2 glass-panel rounded-lg px-4 py-2 flex items-center gap-3 flex-wrap">
              ...
              <span className="text-xs font-mono text-zinc-300" style={{ userSelect: "all", cursor: "text" }}>
                {jobId}
              </span>
              <button
            ...
            {showResume && (
              <div className="mt-2 glass-panel rounded-lg px-4 py-3 flex gap-2 items-center flex-wrap">
                <input
                  ...
                  className="bg-zinc-900 border border-zinc-700 rounded px-3 py-1.5 text-xs
                    font-mono text-zinc-100 flex-1 min-w-[260px] focus:outline-none focus:border-zinc-500"
                />
                <button
          ...
          {/* Watchlist — mobile only (sidebar hidden on mobile) */}
          ...
          {/* Mobile status */}
          {isMobile && status && (
            <p className="mt-2 text-xs font-mono text-zinc-600 truncate">{status}</p>
          )}
          ...
          {tokenUsage.total > 0 && (
            <div className="mt-4 glass-panel rounded-lg overflow-hidden">
              ...
              >
                <span className="text-xs text-zinc-500 uppercase tracking-widest font-mono">Token Usage</span>
          ...
          {/* Mobile session history (tablet/desktop uses sidebar) */}
          {isMobile && sessionHistory.length > 0 && (
            <div className="mt-4 glass-panel rounded-lg overflow-hidden">
              <button
                onClick={() => setShowSessionHistory(b => !b)}
                className="w-full px-4 py-2.5 text-left flex justify-between items-center hover:bg-zinc-800/30 transition-colors"
                  ...
                  {sessionHistory.map((run, i) => {
                    ...
                    return (
                      <div key={i}>
                        <button
                          onClick={() => setExpandedHistoryRow(isExpanded ? null : i)}
                          className="w-full text-left px-3 py-2 rounded font-mono text-xs
                            flex flex-wrap gap-x-2 gap-y-0.5 hover:bg-zinc-800/40 transition-colors"
                        >
                          <span className="text-zinc-200 font-bold">{run.ticker}</span>
                        ...
                        {isExpanded && (
                          <div className="mx-3 mb-1 px-3 py-2 rounded bg-zinc-900/60
                            text-xs font-mono text-zinc-400 whitespace-pre-wrap max-h-32 overflow-y-auto">
                            {run.tokens.byAgent && Object.keys(run.tokens.byAgent).length > 0 ? (
                              Object.entries(run.tokens.byAgent).map(([agent, u]) => (
      ...
      {isMobile && logs.length > 0 && (
        <>
          <div
            className="fixed bottom-0 left-0 right-0 z-50 bg-zinc-950/95 backdrop-blur-md
              border-t border-zinc-800 px-4 py-2 flex items-center gap-2 cursor-pointer"
            onClick={() => setLogSheetOpen(true)}
          >
